...
import {
  ArrowLeft, Plus, X, ChevronUp, ChevronDown,
  Video, Image, AlignLeft, List, Layers, Loader2, Eye,
} from 'lucide-react'
            ...
            {section.videos.map((v, i) => (
              <div key={i} className="flex gap-2">
                <input className={`${inp} flex-1`} placeholder="https://youtube.com/watch?v=..."
                  value={v.url}
                  onChange={e => { const videos = [...section.videos]; videos[i] = { ...videos[i], url: e.target.value }; update({ videos }) }} />
                <input className="w-32 text-sm border border-[#E0DDD8] rounded-xl px-3 py-2.5 outline-none focus:border-[#E05C00] bg-white placeholder-[#BBB]"
                  placeholder="Caption" value={v.caption || ''}
                  onChange={e => { const videos = [...section.videos]; videos[i] = { ...videos[i], caption: e.target.value }; update({ videos }) }} />
            ...
            {section.images.map((img, i) => (
              <div key={i} className="flex gap-2">
                <input className={`${inp} flex-1`} placeholder="https://..."
                  value={img.url}
                  onChange={e => { const images = [...section.images]; images[i] = { ...images[i], url: e.target.value }; update({ images }) }} />
                <input className="w-32 text-sm border border-[#E0DDD8] rounded-xl px-3 py-2.5 outline-none focus:border-[#E05C00] bg-white placeholder-[#BBB]"
                  placeholder="Caption" value={img.caption || ''}
                  onChange={e => { const images = [...section.images]; images[i] = { ...images[i], caption: e.target.value }; update({ images }) }} />
            ...
            {section.items.map((item, i) => (
              <div key={i} className="flex gap-2">
                <input className={`${inp} flex-1`} placeholder="Bullet point text…"
                  value={item}
                  onChange={e => { const items = [...section.items]; items[i] = e.target.value; update({ items }) }} />
...
export default function EditProduct() {
  ...
  async function save() {
    ...
    const { error: err } = await db.from('products').update({
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#F5F2EC] flex items-center justify-center">
      <Loader2 className="w-6 h-6 animate-spin text-[#E05C00]" />
    </div>
  )

  if (!product) return (
    <div className="min-h-screen bg-[#F5F2EC] flex items-center justify-center text-[#888]">
      Product not found.
    </div>
  )
    ...
    <div className="min-h-screen bg-[#F5F2EC]" style={{ fontFamily: 'var(--font-inter), sans-serif' }}>
      <header className="bg-white border-b border-[#E0DDD8] px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-[#111]">Edit Product</span>
          <span className="text-xs text-[#888] font-mono">/{form.slug}</span>
        </div>
        <div className="flex items-center gap-3">
          <Link href={`/${form.slug}`} target="_blank"
            className="flex items-center gap-1.5 text-sm text-[#888] hover:text-[#111] transition px-3 py-2">
        ...
        <Card title="Basic info">
          ...
          <div>
            ...
            <textarea rows={6} className={`${inp} resize-none`}
              placeholder="Write the full product description here. Supports line breaks. This is what appears in the About section on the product page."
              value={form.description}
              onChange={e => setForm(p => ({ ...p, description: e.target.value }))} />
            <p className="text-[11px] text-[#AAA] mt-1">Supports line breaks. Replaces the About section text on the product page.</p>
          </div>
          <div>
            <label className="block text-[11px] font-semibold tracking-wider uppercase text-[#888] mb-1.5">
              SuperProfile.bio URL (embed)
            </label>
            <input className={inp} placeholder="https://superprofile.bio/vp/..." {...f('superprofile_url')} />
            <p className="text-[11px] text-[#AAA] mt-1">
              Embeds a SuperProfile.bio preview below the About section. Example: https://superprofile.bio/vp/🇺🇸-2000--usa-long-cartoon-video-bundle
            </p>
          </div>
